...
class Highlight {
  ...
  Result _parse(
    ...
    dynamic keywordMatch(Mode mode, RegExpMatch match) {
      var matchStr = langMode.caseInsensitive == true
          ? match[0]!.toLowerCase()
          : match[0];
    ...
    int processLexeme(String buffer, [String? lexeme]) {
      ...
      var newMode = _subMode(lexeme, top!);
      ...
      var endMode = _endOfMode(top!, lexeme);
      if (endMode != null) {
        var origin = top!;
        if (origin.skip == true) {
          modeBuffer += lexeme;
      ...
      if (isIllegal(lexeme, top)) {
        throw 'Illegal lexeme "$lexeme" for mode "${top!.className ?? '<unnamed>'}"';
      }
  ...
  Result _parseAuto(String source, {List<String>? languageSubset}) {
    ...
    for (var language in languageSubset) {
      ...
      if (lang == null || lang.disableAutodetect == true) continue;
